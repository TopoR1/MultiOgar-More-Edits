...
UpdateNodes.prototype.writeUpdateItems4 = function (writer) {
    ...
    let scrambleY = this.playerTracker.scrambleY;
    let scrambleId = this.playerTracker.scrambleId;

    for (let i = 0; i < this.updNodes.length; i++) {
        let node = this.updNodes[i];
        if (node.nodeId == 0)
            continue;
        let cellX = node.position.x + scrambleX;
        let cellY = node.position.y + scrambleY;
